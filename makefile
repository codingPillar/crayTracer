...
$(WEB_OUTNAME):$(WEB_VIEW_SRC_OBJ)
	clang --target=wasm32 --no-standard-libraries -Wl,--export-all -Wl,--no-entry -Wl,-allow-undefined-file $(WEB_VIEW_SRC_DIR)/wasm.syms -Wl,--import-memory -o $(WEB_VIEW_SRC_DIR)/$(WEB_OUTNAME).wasm $^
